...
const getListings = async (req, res) => {
  try {
    ...
    const listings = await Listing.find({ ...keyword, status: "Available" })
...
const createListing = async (req, res) => {
  ...
  // Corrected the scope - check role first
  if (req.user.role !== "Seller") {
    return res
      .status(403)
      .json({ message: "User is not a seller. Forbidden." });
  }
  ...
  try {
    ...
    res
      .status(400)
      .json({
        message: "Invalid data provided or server error",
        error: error.message,
      });
  }
};
